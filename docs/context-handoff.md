...
- **Complaint-to-Case Promotion Workflow**: Implemented transactional assignment endpoint (`POST /api/complaints/:complaintId/assign`) automatically promoting complaints to enforcement Cases (`CASE-XXXXXXXXXXXX`), linking via `cases.primary_complaint_id` and `case_complaints`, setting status to `'Assigned'`, and logging to `case_audit_history`. Idempotent logic reuses existing cases. `GET /api/complaints` enriched with subqueried `caseId`.
- **Shared Navigation Helper & Dynamic Routing**: Created `getComplaintAction(complaint)` in `Frontend/src/shared/utils/complaintNavigation.ts`. Dashboard and Complaints list tables dynamically route to `/cases/:caseId` ("View Case →") for promoted complaints or `/complaints/:complaintId` ("View Complaint →") for unassigned complaints.
- **Dashboard Bar Chart Dynamic Headroom**: Calibrated BarChart Y-axis using dynamic headroom formula `Math.ceil(maxZoneCount * 1.35)` with fallback in `DashboardPage.tsx`, removing hardcoded clipping excess.
- **Complaints Page Categorization & Case Link**: Added category filter tabs (`All Complaints`, `Active / Unassigned`, `Assigned / Converted to Case`, `Resolved / Closed`) and a dedicated Case File column with direct badge links to `/cases/:caseId`.
- **Cases Page Multi-Criteria Filtering & Metric Segregation**: Added Lifecycle Status filter (`All Cases`, `Active`, `Pending`, `Solved / Completed`) and Source filter (`All Sources`, `Complaint Based`, `Proactive Field Visit`). Segregated completed/closed cases from active case counters in summary cards.
- **Complaint Detail Page Case Banner & Actions**: Added a Promoted Enforcement Case File banner when a case exists, and an "Assign BI & Create Case" action trigger for unassigned complaints.
- **Construction Status Form Dual Lookup**: Fixed default placeholder `-- Choose an Existing Case or enter Complaint ID --`, removed auto-selection of first case, and added dual input lookup. Updated `GET /api/cases/:caseId` to resolve by `case_id`, `primary_complaint_id`, or `case_complaints.complaint_id`.
- **Display & Text Scaling Controls (Option 2 & Option 4)**: Replaced static font buttons with a functional `Aa` text scaling popover in `Topbar.tsx` featuring a step slider (85% to 115%), percentage readout, and preset buttons. Added display scaling controls to `/settings` page (`SettingsPage.tsx`) with root font-size scaling and `localStorage` persistence.
