    ...
    try {
       ...
       LIMIT 1`, [caseId]);
        ...
        ORDER BY fv.submitted_at DESC`, [actualCaseId]);
            const partsResult = await database_1.pool.query(`SELECT * FROM construction_parts WHERE construction_status_id IN (
    ...
    try {
        ...
        if ((!officerId || !officerName) && caseCheck.rows.length > 0) {
            ...
            if ((!officerId || !officerName) && caseRow.primary_complaint_id) {
                const cmpRes = await client.query("SELECT assigned_officer_id, assigned_officer_name FROM complaints WHERE complaint_id = $1 LIMIT 1", [caseRow.primary_complaint_id]);
            ...
            if ((!officerId || !officerName) && (caseRow.zone || caseRow.block)) {
                const bi = await (0, officerMapping_1.findResponsibleOfficer)(caseRow.zone, caseRow.block, "BI");
                if (bi) {
        ...
        const csInsert = await client.query(`INSERT INTO construction_status (case_id, construction_type, overall_status, created_by_id, created_by_name, updated_at)
         ...
         ON CONFLICT (case_id) DO UPDATE SET
           ...
           updated_at = NOW()
         RETURNING construction_status_id`, [actualCaseId, status, "in_progress", officerId, officerName]);
        ...
        const existingPartsRes = await client.query(`SELECT * FROM construction_parts WHERE construction_status_id = $1`, [constructionStatusId]);
        ...
        if (processNonCompoundable && (status === "non_compoundable" || hasNoticeData)) {
           ...
           ON CONFLICT (construction_status_id, part_type) DO UPDATE SET
              ...
              updated_at = NOW()`, [constructionStatusId, nonCompoundablePartStatus, notice269Id, officerId, officerName]);
